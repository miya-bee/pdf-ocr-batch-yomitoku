...
APP_TITLE = "PDF OCR 付加バッチ（日本語 / YomiToku・縦書き右→左対応）"
OUTPUT_SUFFIX = "_ocr"
...
class YomiTokuRunner:
    ...
    def build_cmd(
        ...
        cmd = (launcher or self._resolve_launcher()) + [
            ...
            "--dpi", str(dpi),
            "-d", device,
            "--reading_order", "right2left",
        ]
        if lite:
            cmd.append("--lite")
        return cmd

    def _needs_pdf_tiff_bridge(self) -> bool:
        """pypdfium2 5系などで PdfDocument.render() が無い環境では、YomiToku の PDF 直入力を避ける。"""
        try:
            import pypdfium2  # type: ignore
        except Exception:
            return False
        try:
            return not hasattr(pypdfium2.PdfDocument, "render")
        except Exception:
            return False

    def _render_pdf_to_multipage_tiff(self, pdf_path: Path, workdir: Path, dpi: int) -> Path:
        """PDF をマルチページTIFFへ変換する。

        YomiToku 側の load_pdf() が壊れている環境でも、TIFF 入力なら load_image() 経由で処理できる。
        """
        try:
            import pypdfium2  # type: ignore
        except Exception as e:
            raise RuntimeError(f"PDFフォールバック用の pypdfium2 を読み込めません: {e}")

        try:
            from PIL import Image  # type: ignore
        except Exception as e:
            raise RuntimeError(f"PDFフォールバック用の Pillow を読み込めません: {e}")

        workdir.mkdir(parents=True, exist_ok=True)
        tiff_path = workdir / "_input_pages.tiff"
        images: list[Any] = []
        doc: Any = None
        ...
        try:
            ...
            if hasattr(doc, "render"):
                rendered = doc.render(pypdfium2.PdfBitmap.to_pil, scale=scale)
                for pil_img in rendered:
            ...
            if not images:
                raise RuntimeError("PDFを画像へ変換できませんでした（ページ画像が0件）。")
            ...
            raise RuntimeError(f"PDF→TIFF 変換に失敗しました: {e}")
        finally:
            try:
    ...
    def run(self, in_pdf: Path, out_pdf: Path, dpi: int, device: str, lite: bool, timeout_sec: int) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="yomitoku_out_") as td:
            ...
            run_input = in_pdf
            if in_pdf.suffix.lower() == ".pdf" and self._needs_pdf_tiff_bridge():
                bridge_dir = outdir / "_bridge_input"
                run_input = self._render_pdf_to_multipage_tiff(in_pdf, bridge_dir, dpi)
                self._log(
                    "WARN",
                    f"[BRIDGE] pypdfium2互換回避のため PDF を一時TIFF化して実行します: {in_pdf.name} -> {run_input.name}",
                )

            cmd = self.build_cmd(run_input, outdir, dpi=dpi, device=device, lite=lite, launcher=launcher)
            ...
            if rc != 0:
                ...
                if "No module named yomitoku.__main__" in detail_all:
                    ...
                    if launcher2 != launcher:
                        cmd2 = self.build_cmd(run_input, outdir, dpi=dpi, device=device, lite=lite, launcher=launcher2)
            ...
            if rc != 0 and in_pdf.suffix.lower() == ".pdf" and run_input == in_pdf:
                detail_all = ((stderr or "") + "\n" + (stdout or "")).strip()
                bridge_needed = (
                    "PdfDocument' object has no attribute 'render'" in detail_all
                    or 'PdfDocument" object has no attribute "render"' in detail_all
                    or ("load_pdf" in detail_all and "Failed to open the PDF file" in detail_all)
                )
                if bridge_needed:
                    bridge_dir = outdir / "_bridge_input"
                    run_input = self._render_pdf_to_multipage_tiff(in_pdf, bridge_dir, dpi)
                    self._log(
                        "WARN",
                        f"[BRIDGE-RETRY] PDF直入力に失敗したため、一時TIFF化して再試行します: {in_pdf.name} -> {run_input.name}",
                    )
                    cmd3 = self.build_cmd(run_input, outdir, dpi=dpi, device=device, lite=lite, launcher=launcher)
                    self._log("WARN", "[RETRY CMD] " + " ".join(self._quote_for_log(x) for x in cmd3))
                    rc, stdout, stderr = self.run_subprocess_with_polling(cmd3, timeout_sec=timeout_sec)
...
class OCRBatchApp:
    ...
    def _build_ui(self) -> None:
        ...
        frm_opts = ttk.LabelFrame(self.root, text="設定（YomiToku / 日本語・縦書き右→左想定）")
    ...
    def _validate_runtime_settings(self, dpi_text: Optional[str] = None) -> tuple[bool, str]:
        if dpi_text is None:
            dpi_text = self.dpi_var.get()
        return self._runtime_support.validate_runtime_settings(dpi_text)
